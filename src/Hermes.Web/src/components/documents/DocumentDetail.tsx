...
export function DocumentDetail({ documentId, onBack }: {
  ...
  return (
    <div className="h-full flex flex-col">
      <button onClick={onBack} className="text-xs text-neutral-500 hover:text-neutral-300 mb-3 self-start">← Back</button>
      ...
      <div className="flex flex-wrap gap-x-6 gap-y-1 text-sm mb-3 text-neutral-400">
        ...
        {doc.extractedAmount != null && <span>💰 ${doc.extractedAmount.toFixed(2)}</span>}
      ...
      <div className={`flex-1 min-h-0 ${canPreview ? 'grid grid-cols-2 gap-4' : ''}`}>
        {/* Left: Extracted content */}
        <div className="flex flex-col min-h-0">
          <div className="text-[10px] font-semibold tracking-widest text-neutral-500 mb-2">EXTRACTED</div>
          {content?.markdown ? (
        ...
        {canPreview && (
          <div className="flex flex-col min-h-0">
            <div className="text-[10px] font-semibold tracking-widest text-neutral-500 mb-2">ORIGINAL</div>
            {ext === '.pdf' ? (
              <iframe
