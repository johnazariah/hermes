...
function extractMonth(d: string | null): string {
    ...
    const m =
        d.match(/(\d{4})-(\d{2})/) || d.match(/(\d{2})\/(\d{2})\/(\d{4})/);
    if (m && m.length >= 3) {
...
function isCleanVendor(v: string | null | undefined): v is string {
    ...
    const junk =
        /your |bank account|financial|institution|credit\/debit|click here|page \d|http/i;
    return !junk.test(v);
}
...
function sortDocs(docs: DocumentSummary[], sortBy: SortBy): DocumentSummary[] {
    return [...docs].sort((a, b) => {
        if (sortBy === "amount")
            return (b.extractedAmount ?? 0) - (a.extractedAmount ?? 0);
        if (sortBy === "name")
...
export function DocumentList({
    ...
    const handleContextMenu = useCallback(
        (e: React.MouseEvent, docId: number) => {
            e.preventDefault();
        ...
        <div onClick={() => setContextMenu(null)}>
            ...
            {selected.size > 0 && (
                <div className="flex items-center gap-3 mb-3 px-3 py-2 bg-blue-950/30 border border-blue-900/50 rounded-lg text-sm">
                    ...
                    <button
                        onClick={() => batchAction("star", "")}
                        className="text-xs text-neutral-400 hover:text-neutral-200 ml-auto"
                    >
                        ⭐ Star
                    </button>
                    <button
                        onClick={() => {
                            const tag = prompt(
                                "Tag name (e.g. rental-property-1, FY2024-25):",
                            );
            ...
            {contextMenu && (
                ...
                >
                    ...
                    <button
                        ...
                        className="w-full text-left px-3 py-1.5 text-sm hover:bg-neutral-700 text-blue-400"
                    >
                        ＋ New category...
                    </button>
                    ...
                    <button
                        ...
                        className="w-full text-left px-3 py-1.5 text-sm hover:bg-neutral-700"
                    >
                        ⭐ Star
                    </button>
                    <button
                        onClick={() => {
                            onSelectDocument(contextMenu.docId);
                            setContextMenu(null);
                        }}
                        className="w-full text-left px-3 py-1.5 text-sm hover:bg-neutral-700"
                    >
        ...
        <div>
            ...
            >
                <input
                    ...
                    onChange={() =>
                        onSelectGroup(allSelected ? [] : docs.map((d) => d.id))
                    }
